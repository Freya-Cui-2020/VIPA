...
=head1 Name
	...
	perl blast_parser_ww.pl -- parse the BLAST result and convert to tabular format.
...
=head1 Usage
	...
	perl blast_parser_ww.pl [options] input_file
...
=head1 Exmple
	...
	perl blast_parser_ww.pl test_chr_123.seq.bgf.pep.1000.10.blast > test_chr_123.seq.bgf.pep.1000.10.blast.tab
	
	2. Run with user specified Parameters:
	perl blast_parser_ww.pl -tophit 2 test_chr_123.seq.bgf.pep.1000.10.blast > test_chr_123.seq.bgf.pep.1000.10.blast.tab	
	perl blast_parser_ww.pl -topmatch 2 test_chr_123.seq.bgf.pep.1000.10.blast > test_chr_123.seq.bgf.pep.1000.10.blast.tab	
	perl blast_parser_ww.pl -tophit 3 -topmatch 2 -eval 1e-5 test_chr_123.seq.bgf.pep.1000.10.blast > test_chr_123.seq.bgf.pep.1000.10.blast.tab
	perl blast_parser_ww.pl -nohead -tophit 3 -topmatch 2 -eval 1e-5 test_chr_123.seq.bgf.pep.1000.10.blast > test_chr_123.seq.bgf.pep.1000.10.blast.tab
...
sub parse_blast{
	...
	#print ($file,$tophit,$topmatch,$eval,$nohead);
	...
	#my $type=<BLAST>;
	#print $type;
	my $type="Effective";#defined newline separetor
	$/=$type;#newline sep is "Query="
	my $database;
	while (<BLAST>) {
		if(/No hits found/){
			next;
		...
		if ($cycle[0]=~/Query= (\S+)\s+(.+?)Length=(\S+)/s) {
			$query=$1;
			$query_annotation=$2;
			$query_len=$3;
			$query_annotation=~s/\n//g;
			$query_annotation=~s/\s{2,}/ /g;
			$query_annotation="--" if($query_annotation eq "");
		}
		...
		for (my $i=0;$cycle[$i];$i++) {
			...
			if ($cycle[$i]=~/(.+?)Length=(\d+)/s) {
				$subject_annotation=$1;
				$subject_len=$2;
				$subject_annotation=~s/\n//g;
				$subject_annotation=~s/\s{2,}/ /g;
				next if($subject_annotation eq " ");
				my @subinfo=split /> /, $subject_annotation;#######change by weiwei at 2014.8.29
				$subject_annotation='';
				foreach my $subannot (@subinfo) {
					if ($subannot=~/unknown|unnamed|hypothetical Protein|predicted Protein|putative|uncharacterized protein/i) {
					}else{
						$subject_annotation.='> '.$subannot;
					}
				}
				next if($subject_annotation eq "");	#信息为空时用"--"代替
			...
			my @cycle_inner=split (/\n Score =/,$cycle[$i]);	#分开同一个query和同一个subject的多个比对结果
			shift @cycle_inner;
			for (my $j=0;$cycle_inner[$j];$j++) {
				...
				# midified by miaoxin 2014-09-05
				$cycle_inner[$j]=~s/Lambda.+?$//s;
		...
		if( ($gethit == 0 ) && ($subject_annotation eq '')){
			...
			my @cycle_inner=split (/\n Score =/,$cycle[0]);	#分开同一个query和同一个subject的多个比对结果
			shift @cycle_inner;
			for (my $j=0;$cycle_inner[$j];$j++) {
				last if((defined $topmatch) && $j>$topmatch-1);
				if ($cycle_inner[$j]=~/(\S+) bits.+?Expect.+?=\s+(\S+),.+?Identities\D+\d+\/(\d+)\s+\((\S+)\%\)/s) {
